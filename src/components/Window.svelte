<script lang="ts">
  ...
  $: positionOffsetX = Math.pow(((center.y - $scrollSpring) / windowHeight - 0.5), 2) * 2000 * side;

  $: side = mobile ? Math.sin(center.y / 3000 - 0.5) : center.x / windowWidth - 0.5; // * Math.sign(center.y / windowHeight - 0.5)
  ...
  $: style = `transform: perspective(1000px) scale(${$size}) rotateY(${rotateY}deg) rotateX(${rotateX}deg) translateX(${positionOffsetX}px); filter: brightness(${brightness})`;
